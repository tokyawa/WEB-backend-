...
class District(models.Model):
    ...
    image_url = models.URLField(verbose_name="Ссылка на изображение", max_length=1000, blank=True,null=True)
...
class Zone(models.Model):
    ...
    priority_summary_cache = models.TextField(null=True, blank=True, verbose_name="Сводка приоритетов")
    ...
    def priority_summary(self):
        districts = self.districts.all()
        priorities = {"high": 0, "medium": 0, "low": 0}
        main_district = None

        for zone_district in districts:
            priorities[zone_district.priority] += 1
            if zone_district.is_main:
                main_district = zone_district.district.district_name

        summary = f"Приоритеты: Высокий: {priorities['high']}, Средний: {priorities['medium']}, Низкий: {priorities['low']}."
        if main_district:
            summary += f" Ключевой район: {main_district}."
        else:
            summary += " Ключевой район не назначен."
